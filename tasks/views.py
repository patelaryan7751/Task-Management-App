...
from tasks.models import Task, TaskEmail
...
class GenericTaskView(AuthorisedTaskManager, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        obj, _ = TaskEmail.objects.get_or_create(
            user=self.request.user)
        context["TaskEmail_id"] = obj.pk
        return context


class TaskMailForm(ModelForm):
    class Meta:
        model = TaskEmail
        fields = ["mail_time"]


class TaskMailView(UpdateView, LoginRequiredMixin):
    form_class = TaskMailForm
    success_url = "/tasks"
    template_name = "task_email.html"

    def get_queryset(self):
        return TaskEmail.objects.filter(user=self.request.user)
